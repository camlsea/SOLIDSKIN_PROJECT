/*
 *    @author: FusionMaps Team (SB)
 *    @description: FusionMaps Export Component
 *
 *    @publish: 17-03-2009
 *    @version: 1.0.0 (build 347)
 */
 ...
 *
 *  1.0 - 20/01/2009
 *  Ported from FusionMaps Export Component
 ...
 *  1. FC_ExportDataReady is called by indivdual maps and ASCII data with
 ...
 *  4. FC_ExportComponentReady is called when individual components are
 *     ready to accept data. When it attains readyness, it checks whether 
 *     there is already any StreamObject queued up in STEP 4. If yes, it
 *     Flushes the entire queue of StreamObjects.
 *
 *  # Error number convention:
 *  1yymmddhhmm
 *
 *  # Why have we not made aliases of FC_ExportDataReady and
 *  FC_ExportComponentReady and instead defined the se functtions?
 ...
 *  well as temporally. (2) Light prototype definition... alloqing for
 *  lesser overhead per new object. (3) Fewer try-catch blocks. (4) Cross-
 *  browser compatible. (5) Co-exist with almost all other JS frameworks. 
 */


//  Check whether FusionMaps main object is already declared or not.
//  Debug Info: In case you get an error, make sure -
//  1. FusionMapsExportComponent.js (this file) is included AFTER the main
//     FusionMaps.js file is included.
//  2. If the parent script has any modifications or errors, the main object
//     may remain undefined
if(typeof infosoftglobal == 'undefined'
    || typeof infosoftglobal.FusionMaps == 'undefined')
{
    alert('FusionMapsExport: object::FusionMaps was not found.'
        + ' Verify script inclusions.', 10901271400);
    throw 'FusionMapsExportComponentFatalError::10901271400';
}
...
/**
 *  Create FusionMapsExportObject class and map an easy to use variable to
 *  this as this is too long of a name to be used as a workset
 *  @var FusionMapsExportObject is set as an easy-access alias for the global
 *  namespace.
 *  @var _FCEO is another alias for easy internal JS space-saving coding.
 */
infosoftglobal.FusionMapsExportObject = function()
{
    // initializeComponent constructor routines (that has been declared
    // separately)
    _FCEO.initializeComponent(this, arguments);
};
var FusionMapsExportObject = infosoftglobal.FusionMapsExportObject;
var _FCEO = infosoftglobal.FusionMapsExportObject;
...
_FCEO.defaultParameters = {
    ...
    componentAttributes: {
        ...
        strictTrigger: '1'  // whether the handlerId is overrideable for maps
    },
    exportAttributes: { },
    sourceMaps: [],
...
/**
 *  Initiates exportChart sequence on all the maps provided in the
 *  sourceMaps array
 *  @return array
 */
_FCEO.prototype.BeginExport = function()
{
    // call the batchExport() function with the  as parameter
    return _FCEO.batchExport(this, this.sourceMaps);
};

/**
 * Initiates exportChart sequence on ALL FusionMaps within the current page
 * @return array
 */
_FCEO.prototype.BeginExportAll = function()
{
    ...
    // 2. call batchExport with above maps

    // call the batchExport() function with the all maps as parameter
    return _FCEO.batchExport( this, _FCEO.probeMaps() );
};
...
/**
 *  This function updates the FusionMaps object within the codes as and when
 *  users update the UIProperties
 *  @throws FusionMapsComponentException is raised when due to some god
 *          forsaken reason, the internal FusionMaps object (this.srcObj)
...
/**
 *  The number of modal maps that are pending to be completely captured (m)
...
try {
	_FCEO.playerVersion = infosoftglobal.FusionMapsUtil.getPlayerVersion();
}
catch (err) {
	_FCEO.trace('FusionMapsExport: object::Flash Player DetectionError.\n' +
        err, 10902182119);
}
/**
...
/**
 *  Parses DOM tree for all existing FusionMaps.
 *  Note: supported for FusionMaps with release 3.1 or higher.
 */
_FCEO.probeMaps = function()
{
    // Internal Algorithm:
    // 1. Parse through objects and accumulate valid FusionMaps
...
/**
 *  Determines whether a particular HTML DOM element is a FusionMaps or not.
 ...
 *  parameters
 *  @param obj FusionMapsExportObject: usually a "this" value sent by the
 *         constructor.
...
_FCEO.initializeComponent = function(obj, args)
{
    ...
    // 3. Create new srcObj (FusionMaps) out of the args
    ...
    // create a new fusionmaps object from the arguments. 
    obj.srcObj = new FusionMaps(obj.swfLocation, obj.id, 0, 0,
		obj.debugMode);
 ...
 *  the parameter
 *  @param obj object: And instance of FusionMapsExportObject, usually 'this'
 */
_FCEO.updateComponent = function(obj) {
    // Internal Algorithm:
    // 1. check if fusionmaps object is created.
    // 2. update componentAttributes and related things in case hook is provided

    // create new fusionmaps object if one is not already created
    if(!obj.srcObj) {
...
/**
 *  Initiates exportMap() command on an array of fusionmaps
 *  @param obj object: FusionMapsExportComponent that will invoke batch
 *  @param src array: The source maps that will be used to run batch on
 *  @return array
 */
_FCEO.batchExport = function(obj, src)
{
    ...
    // 3. Call exportChart on sourceMaps

    // check whether sourceMaps is a valid array
    if( !(src instanceof Array) ) {
        obj.trace(_FCEO.R.errArgs, 10902091853);
        return [];
    }

    // accumulator variable to store dom ids of successfully exported maps
    ...
    for(var i = 0; i < src.length; i++) {
        ...
        // initiate exportMap() command within an if block so that any
        // external error can be trapped with ease
        if(cObj && cObj.exportChart) {
            cObj.exportMap(obj.exportAttributes);
            accArr.push(cObj.id);
        }
        else {
...
_FCEO.R = {
    ...
    errFusion: 'Error with internal FusionMaps object. Review parameters or '+
        'Global Security Settings.',
    errPlayer: 'Incompatible Flash player or Flash player not installed. ' +
        'Flash Player 10 (or above) is needed for Export Component.',
	msgNoPlayer: 'This page contains elements that require the latest ' +
        'version of Flash Player plugin.',
    resSig: 'FusionMapsExportComponent',
    resChartSig: 'FusionMaps/'
};
